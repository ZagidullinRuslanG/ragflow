...
`,
      ...
      additionalParsingInfo: 'Доп. параметры',
      additionalParsingInfoTip:
        'Допустимые параметры:<br><b>--lower_boundary</b> 6.7 - определяет отступ снизу документа, %;<br>--upper_boundary 4.0 - определяет отступ сверху документа, %',
      additionalParsingInfoInputField:
        'Введите дополнительную информацию для парсинга...',
      additionalParsingInfoTooltip:
        '<b>Дополнительная информация для парсинга.</b>',
    },
...
<ul>
  ...
  <li>Функция авто-ключевых слов зависит от LLM и потребляет значительное количество токенов.</li>
    ...
    chat: {
      ...
      knowledgeBasesSec: 'Вторичные базы знаний',
      ...
      modelContextConstSizeTip:
        'Укажите любое число, отличное от 0, чтобы задать постоянное окно контекста. Укажите 0, чтобы система динамически определяла окно контекста. Сильное влияние на VRAM!',
      retryTempShift: 'Смещение температуры',
      retryTempShiftMessage: 'Смещение температуры',
      retryTempShiftTip:
        'Смещение температуры при повторных попытках ответа в формате вида "0.1, 0.2" - при первой попытке ответа t = настроенной температуре во вкладке "Настройки модели" (если не настроено, то = 0), при втором t = 0.1, при третьем t = 0.2, при последующих t = 0.2',
      retryTimeoutShift: 'Смещение таймаута',
      retryTimeoutShiftMessage: 'Смещение таймаута',
      retryTimeoutShiftTip:
        'Смещение задержки перед запросом при повторных попытках ответа в формате вида "0, 0.1, 0.2" - при первой попытке ответа To = 0, при втором To = 0.1, при третьем To = 0.2, при последующих To = 0.2',
      showRetries: 'Показывать неудачные попытки',
      showRetriesMessage: 'Показывать неудачные попытки',
      showRetriesTip:
        'Показывать в ответе от модели неудачные попытки ответа с обнаруженными циклами',
      retriesCount: 'Количество попыток',
      retriesCountMessage: 'Количество попыток',
      retriesCountTip:
        'Сколько раз модель попробует ответить снова, если обнаружится циклический ответ. При значении 0 не будет делать повторных запросов.',
      loopMinChars: 'Мин. кол-во символов для цикла',
      loopMinCharsMessage: 'Мин. кол-во символов для цикла',
      loopMinCharsTip:
        'Минимальное количество символов которое будет использоваться для определения цикличности. Базовое значение - 50',
      loopThresh: 'Макс. кол-во повторов',
      loopThreshMessage: 'Макс. кол-во повторов',
      loopThreshTip:
        'Максимальное количество обнаруженных повторов, прежде чем ответ будет определен как цикличный. Базовое значение - 3.',
      promptSuffixOnRetry: 'Суффикс при повторе',
      promptSuffixOnRetryMessage: 'Суффикс при повторе',
      promptSuffixOnRetryTip:
        'Строка, которая будет добавляться в конец промпта при повторной попытке запроса.',
      checkEveryN: 'Проверка каждые № токенов',
      checkEveryNMessage: 'Проверка каждые № токенов',
      checkEveryNTip:
        'Проверка цикличности каждые № токенов. Базовое значение - 8.',
      showThinking: 'Показывать раздумывания',
      showThinkingMessage: 'Показывать раздумывания',
      showThinkingTip:
        'Показывает в чате этап thinking у моделей (если он поддерживается)',
    },
    setting: {
      edit: 'Редактировать',
      cropTip:
...
Пример: Virtual Hosted Style`,
  ...
  Выше приведено содержимое, которое вам нужно обобщить.`,
      ...
      answer2: 'Взаимодействие 2',
      answer2Description:
        'Компонент для взаимодействия между человеком и ботом (Версия 2). Используется для вывода финального ответа пользователю. Так же используется для указания базы данных, настройки которой будут использоваться для парсинга документов (если в ходе чата пользователь прикрепит документ).',
